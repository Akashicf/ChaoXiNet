...
optim_evo = torch.optim.Adam(EvolutionNet.parameters(), lr=10e-3, betas=(0.5,0.999))
...
num_epochs = 200  # 例如您想训练多少个 epoch
test_interval = 100
...
for epoch in range(num_epochs):
    ...
    for batch_id, test_ims in enumerate(loader):
        ...
        evo_result_detach = evo_result.detach() / 128
        ...
        if (global_step + 1) % test_interval == 0:
...
def get_masked_array(data):
    # 将所有值为0的点掩码掉
    return np.ma.masked_less(data, 1)
